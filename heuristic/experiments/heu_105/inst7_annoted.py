        ...
        for line in f:
            if line.find('xdeltime') >= 0 and len(cli_data) < 6:
               ...
               cli_data.append(float(data[2:]))
                 ...
                 exp_inst2.append(n_inst[1:])
                 #horizon.append(hrz)
                 exp_data2.append(float(data2[2:]))
            if re.search(r'\-{10}',line) != None and len(exp_data1) > 0 and len(exp_data2) > 0 and len(cli_data) > 0:
               #print(exp_data)
               #print(cli_data)
               instances = exp_inst1 + exp_inst2 + cli_inst
               print(exp_inst1)
               print(exp_inst2)
               print(cli_inst)
               print(instances)
               plot(horizon,name1,name2,'clingcon',exp_data1,exp_data2,cli_data,opt1,opt2,name_figure,instances) if str(flag) == "target" else \
               plot(horizon,flag,name2,'clingcon',exp_data1,exp_data2,cli_data,opt1,opt2,name_figure,instances) 
               horizon.clear()
               cli_data.clear()
               exp_data1.clear()
               exp_data2.clear()
